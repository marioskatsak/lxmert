...
from tasks.blocks_model import ROSMIModel
from tasks.blocks_data import ROSMIDataset, ROSMITorchDataset, ROSMIEvaluator
...
class ROSMI:
    def __init__(self,get_data_tuple):
        ...
        self.writer = SummaryWriter(f'snap/blocks/logging_rosmi_{args.n_ent}_names/{os.uname()[1]}.{time.time()}')
        # Model
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            for i, (sent_id, feats, feat_mask, boxes, names, sent, dists, diste, land_, cland_, bear_ , world_st, eu_dist_ed_co, stacked_ew_ns, l_start,l_end, target) in iter_wrapper(enumerate(loader)):
                ...
                for sid,diss,dise,ln,ln_,br,l_s,l_e, l in zip(sent_id, dist_s.cpu().detach().numpy(), \
                    ...
                    # print(ans)
                    sentid2ans[sid] = (l, diss,dise, ln,ln_, br,l_s,l_e)
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                for qid,diss,dise, ln,cln, br,l_s,l_e, l in zip(ques_id,dist_s.cpu().detach().numpy(), \
                    ...
                    br = dset.label2bearing[br]
                    sentid2ans[qid] = (l.tolist(), int(diss), int(dise), ln.tolist(),int(cln), br, int(l_s), int(l_e))
