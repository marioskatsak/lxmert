...
class ROSMIEvaluator:
    ...
    def evaluate(self, sentid2ans: dict):
        ...
        for sentid, (pred_box, diss,dise, ln, ln_, br, l_s,l_e) in sentid2ans.items():
            ...
            examples.append({ 'id':sentid, 'img_id':datum['img_id'], 'sentence':sent, 'gold':[landmark_id_,str(datum['landmarks'][0]['distance'])+' '+str(dists)+ ' '+str(diste),datum['landmarks'][0]['bearing']], 'pred':[ln_,str(diss)+ ' '+str(dise),br], 'outcome':siou3 > thres })
